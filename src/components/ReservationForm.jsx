...
function PopupForm({post}) {
  ...
  async function sendReservationDetails(customerName,customerEmail,reservationDate){
    const formData={
          ...
          <div style={popupStyle}>
            ...
            <form onSubmit={handleFormSubmit}>
              <div>
                ...
                <input type="text" id="name" name="name" required ref={customerNameElement}/>
              </div>
              <div style={{ marginTop: '10px' }}>
                <label htmlFor="email">Email:</label>
                <input type="email" id="email" name="email" required ref={customerEmailElement} />
              </div>

              <div style={{ marginTop: '10px' }}>
                <label htmlFor="reservationDay">Reservation day:</label>
                <input type="date" id="reservationDay" name="reservationDay" required ref={reservationDayElement}/>
              </div>

              <div style={{ marginTop: '10px' }}>
                <label htmlFor="bookedService"><b>Service Choosen: </b>{post.serviceTitle}</label>
                
              </div>

              <div style={{ marginTop: '10px' }}>
                <label htmlFor="bookedService"><b>Total: </b>{post.pricing}€</label>
                
              </div>

              <div style={{ marginTop: '20px' }}>
                <button type="submit">Confirm</button>
                <button type="button" onClick={closePopup} style={{ marginLeft: '10px' }}>
                  Cancel
                </button>
              </div>
            </form>
